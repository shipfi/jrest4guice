...
class BinderImpl implements Binder {
	...
	private static final Logger logger = Logger.getLogger(BinderImpl.class
			.getName());

	final List<BindingBuilderImpl<?>> bindingBuilders = new ArrayList<BindingBuilderImpl<?>>();
	...
	private static final InternalFactory<Injector> INJECTOR_FACTORY = new InternalFactory<Injector>() {
	...
	private static final InternalFactory<Logger> LOGGER_FACTORY = new InternalFactory<Logger>() {
		...
		public Logger get(InternalContext context) {
			...
			return member == null ? Logger.getAnonymousLogger() : Logger
					.getLogger(member.getDeclaringClass().getName());
		}
	 ...
	 * 
	 * @param stage
	 *            we're running in. If the stage is {@link Stage#PRODUCTION}, we
	...
	public void bindInterceptor(Matcher<? super Class<?>> classMatcher,
		...
		proxyFactoryBuilder
				.intercept(classMatcher, methodMatcher, interceptors);
	}
	...
	public <T> BindingBuilderImpl<T> bind(Key<T> key) {
		BindingBuilderImpl<T> builder = new BindingBuilderImpl<T>(this, key,
				source());
	...
	public ConstantBindingBuilderImpl bindConstant() {
		ConstantBindingBuilderImpl constantBuilder = new ConstantBindingBuilderImpl(
				this, source());
	 ...
	 *             if configuration errors are found. The expectation is that
	 *             the application will log this exception and exit.
	...
	Injector createInjector() throws CreationException {
		...
		injector = new InjectorImpl(proxyFactoryBuilder.create(), bindings,
				scopes);
		...
		final List<ContextualCallable<Void>> preloaders = new ArrayList<ContextualCallable<Void>>();
	...
	private <T> void createBinding(BindingBuilderImpl<T> builder,
		...
		final InternalFactory<? extends T> factory = builder
				.getInternalFactory(injector);
		BindingImpl<?> binding = BindingImpl.newInstance(injector, key, builder
				.getSource(), factory);
	...
	private static Set<Class<?>> forbiddenTypes() {
		...
		Collections.addAll(
				...
				// It's unfortunate that we have to maintain a blacklist of
				// specific
				// classes, but we can't easily block the whole package because
				// of
				// all our unit tests.

				AbstractModule.class, Binder.class, Binding.class, Key.class,
	...
	void putBinding(BindingImpl<?> binding) {
		...
		if (FORBIDDEN_TYPES.contains(rawType)) {
			addError(binding.getSource(),
					ErrorMessages.CANNOT_BIND_TO_GUICE_TYPE, rawType
							.getSimpleName());
			return;
		}

		if (bindings.containsKey(key)) {
			addError(binding.getSource(), ErrorMessages.BINDING_ALREADY_SET,
					key, original.getSource());
		} else {
	...
	static class RuntimeErrorHandler extends AbstractErrorHandler {
		...
		public void handle(Object source, String message) {
			throw new ConfigurationException("Error at " + source + " "
					+ message);
		}
	}
	...
	class StaticInjection {
		...
		final Object source;
		final Class<?>[] types;
		final List<SingleMemberInjector> memberInjectors = new ArrayList<SingleMemberInjector>();
			...
			injector.withDefaultSource(source, new Runnable() {
				public void run() {
					for (Class<?> clazz : types) {
						injector.addSingleInjectorsForFields(clazz
								.getDeclaredFields(), true, memberInjectors);
						injector.addSingleInjectorsForMethods(clazz
								.getDeclaredMethods(), true, memberInjectors);
					}
				}
			});
		}
	...
	static class BindingPreloader implements ContextualCallable<Void> {
		...
		public Void call(InternalContext context) {
			ExternalContext<?> externalContext = ExternalContext.newInstance(
					null, key, context.getInjectorImpl());
